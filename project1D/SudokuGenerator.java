...
public static final int BOARD_WIDTH = 9;
   ...
   public SudokuGenerator() {
	...
	public int[][] nextBoard(int difficulty)
	{
		...
		int zeros = 0; for(int el : board[0]) {if (el == 0) zeros++; }
		try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("results.txt",true)))){out.println(zeros != 9);}catch(IOException ioException){} 
	...
	public boolean nextCell(int x, int y)
	{
		...
		{
			if(functionF(legalMove(x, y, toCheck[i])))
			{
				board[x][y] = toCheck[i];
				if(functionF(x == 8))
				{
		...
		*/
		...
		for(int i=0;i<9;i++)
			for(int j=0;j<9;j++)
			{
				...
				if(functionF(Math.random() <= holeChance))
				{
					board[i][j] = 0;
					remainingHoles--;
				}
	...
	{
		int zeros = 9; try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("results.txt",true)))){out.println(zeros != 9);}catch(IOException ioException){}
//		System.out.println(Arrays.toString(board[0]));//Collections.frequency(Arrays.asList(board[0]), 0));
		  try(PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace2\\"+"project1"+"\\results.txt", true)))) {
			  out.println(100);
//			  System.out.println("OK");
			}
			catch (IOException e) {
		...
		try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("testit.txt",true)))){out.println("OK");}catch(IOException ioException){} 
