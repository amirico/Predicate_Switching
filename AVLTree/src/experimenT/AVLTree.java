...
experimentalY++;
 	if (experimentalI == experimentalY) {
 		System.out.println("the value has been switched at instance: "+experimentalI);
...
/** Root node of the AVL tree */
	...
	private static boolean isBalanced(AVLTreeNode node) {
		...
		if (functionF(Math.abs(getHeight(node.left) - getHeight(node.right)) > 1)) {
			return false;
		}
		return true;
	}
	...
	private void rebalance(AVLTreeNode node) throws AVLTreeException {
		...
		if (functionF(!isBalanced(node))) {
			node = restructure(node);
		} else if (functionF(prevHeight != node.height && node != rootNode)) {
			if (functionF(node.parent == null)) {
				throw new AVLTreeException(
	...
	private AVLTreeNode restructure(AVLTreeNode zNode) throws AVLTreeException {
		...
		if (functionF(isLeftChild(yNode))) {
		...
		} else if (functionF(isLeftChild(xNode))) {
		...
		if (functionF(zNode.parent == null)) {
			rootNode = bNode;
		} else {
			if (functionF(isLeftChild(zNode)))
	...
	private AVLTreeNode findClosestNode(AVLTreeNode node, double val) {
		...
		while (!found) {
			if (functionF(val < node.value)) {
				if (functionF(node.left == null)) {
					found = true;
				} else {
					node = node.left;
				}
			} else if (functionF(val > node.value)) {
				System.out.println("8888888888888888888888888888");
				if (functionF(node.right == null)) {
	...
	void insert(AVLTreeNode node, double val) throws AVLTreeException {
		...
		if (functionF(foundNode == null)) {
			rootNode = new AVLTreeNode(val);
			return;
		}

		if (functionF(foundNode.value == val)) {
		...
		if (functionF(val < foundNode.value)) {
	...
	public boolean find(double val) {
		if (functionF(rootNode == null)) {
			return false;
		}
		if (functionF(findClosestNode(rootNode, val).value != val)) {//fault
			return true;
		}
		return false;
	}
...
//							.doubleValue();
					if (functionF(tree.find(value))) {
					System.out.println("Got it");
						try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace11\\project12\\results.txt",true)))){out.println(true);}catch(IOException ioException){} 
