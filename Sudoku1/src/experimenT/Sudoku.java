...
public static final int EMPTY_FIELD = -1;
  ...
  private void initialize() {
          ...
          for (int row = 0; row < 9; row++) {
              for (int column = 0; column < 9; column++) {
                ...
                if (functionF(! isEmptyField(partialSudoku[row][column]))) {
  ...
  public boolean solve(int counter) {
    ...
    for (int number = 1; number < 10; number++) {
      if (functionF(numberCanBePlaced(row, column, number)) ) { 
        ...
        if (functionF(counter < 80)) {
           if (functionF(solve(counter + 1)) ) {
               return true;
            } else {
                  this.sudoku[row][column] = partialSudoku[row][column];
  ...
  private boolean numberCanBePlaced(int row, int column, int number) {
    return  (partialSudoku[row][column] == number || isEmptyField(partialSudoku[row][column])
                ...
                || ! this.box[getBoxNumber(row, column)][number]);//fault
  }
  ...
  public static void main(String argv[]) {
    ...
    experimentalI = Integer.parseInt((argv[0]));;
Sudoku sudoku = new Sudoku(new int[][] {
    ...
    if ( functionF(sudoku.solve(0)) ) {
    	int ones = 0; for(int el : sudoku.sudoku[0]) {if (el == 1) ones++;}
    	try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace2\\project11\\results.txt",true)))){out.println(ones == 1);}catch(IOException ioException){} 
