...
int[][] matrix = parseProblem(args);
        ...
        if (solve(0,0,matrix))    {// solves in place
            try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace11\\Sudoku2\\results.txt",true)))){out.println(true);}catch(IOException ioException){} 
            writeMatrix(matrix);}
        else
    ...
    static boolean solve(int i, int j, int[][] cells) {
        ...
        if (functionF(cells[i][j] != 0))  // skip filled cells
            return solve(i+1,j,cells);

        for (int val = 1; val <= 9; ++val) {
            if (functionF(legal(i,j,val,cells))) {
                cells[i][j] = val;
                if (functionF(solve(i+1,j,cells)))
                    return true;
            }
        }
    ...
    static boolean legal(int i, int j, int val, int[][] cells) {
        for (int k = 0; k < 9; ++k)  // row
            if (functionF(val == cells[k][j]))
                return false;

        for (int k = 0; k < 9; ++k) // col
            if (functionF(val == cells[i][k]))
                return false;
        ...
        for (int k = 0; k < 3; ++k) // box
            for (int m = 0; m < 3; ++m)
                if (functionF(val == cells[boxRowOffset+k][boxColOffset+m]))
                    return false;
    ...
    static void writeMatrix(int[][] solution) {
        for (int i = 0; i < 9; ++i) {
            if (functionF(i % 3 == 0))
                System.out.println(" -----------------------");
            for (int j = 0; j < 9; ++j) {
                if (functionF(j % 3 == 0)) System.out.print("| ");
