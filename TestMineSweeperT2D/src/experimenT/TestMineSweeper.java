...
/** 
  ...
  */
  ...
  {
    if (functionF(status == "play" && validIndex(r,c))) 
    {
        //IF tile is clear set to clear
        if (getTiles(r,c) == 0)
            tiles[r][c] = 0;
        if (functionF(tiles[r][c] == 1 || tiles[r][c] == 2)) 
        {
            tiles[r][c] = t;
            ...
            //CHECK FOR GAME WIN
            if (functionF(tiles[r][c] == 3 && mines[r][c] == 9)) 
            {
                cFlags++;
                gameWon();
            }
            
            if (functionF(mines[r][c] == 0 && t == 0)) 
            {
        ...
        if (functionF(tiles[r][c] == 3))//DONT UNCOVER FLAGGED TILES
        {
            if (t != 0)
                if (mines[r][c] == 9 && tiles[r][c] == 3 && t == 2) //DECREMENT NUM FLAGS IF SWITCHING TO QUESTIONMARK
                {
		  cFlags--;
		 }
                tiles[r][c] = t;
  ...
  */
  public char getBoard (int r, int c) 
  {
    ...
    if (functionF(status == "lose")) { //HANDLE LOSE STATUS DRAWING
        if (tiles[r][c] == 0)
        {
    ...
    if (functionF(status == "play")) //HANDLE PLAY DRAWING
    {
        ...
        if (functionF(tiles[r][c] == 0)) //IF OPEN TILE
        {
    ...
    if (functionF(status == "win"))//HANDLE DRAWING BOARD POST-WIN
    {
  ...
  */
  private void initGame (int newRows, int newCols) 
  {
    ...
    if (functionF((newRows >= 1) && (newCols >= 1))) 
    {
  ...
  */
  private void placeMines () 
  {
    ...
    for (int i = 0; i < cMines; i++)
    {
        ...
        if (functionF(mines[r][c] == 9)) //IF RANDOM PLACE IS ALREADY MINE DEC I AND DO IT AGAIN
        {
            i--;
            continue;
        }
...
// create new minesweeper instance 2 rows by 5 columns
    ...
    // display board
    System.out.println ( game.toStringBoard() );
 try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace2\\project92T\\results.txt",true)))){out.println(true);}catch(IOException ioException){} 
} 
}