...
try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace2\\project2\\results.txt",true)))){out.println(ones == 1);}catch(IOException ioException){} 
 *
...
int[][] matrix = parseProblem(args);
        writeMatrix(matrix);
        if (functionF(solve(0,0,matrix)))  {  // solves in place
            writeMatrix(matrix);
