...
public static boolean checkValidity(String input){
			...
			if (functionF(checkLength < 2 && checkLength > 6)) { // it's an appropriate length
				// check for digits
				if (! Character.isDigit(input.charAt(checkLength-1))) { //check last character
					valid = false;
				} else if (! Character.isDigit(input.charAt(checkLength-2))) { //check second to last
					valid = false;
				} else if (! Character.isDigit(input.charAt(checkLength-3))) { // check third to last
					valid = false;
				}
			} else {
				valid = false;
...
// we need to get input ... for now we'll use the Scanner class
		...
		try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace11\\BMICalculator4\\results.txt",true)))){out.println((bmi == 4.9991111111111115));}catch(IOException ioException){} 
