...
public static boolean trace (boolean b){return b;}
	public static void main(String[] args) throws Exception
  {
	...
	for (int y = 0; y < 9; y++)
	{
	  for (int x = 0; x < 9; x++)
	  {
	    ...
	    if (trace(x == 8 && fileScanner.hasNextLine()))
		{
  ...
  {         
    ...
    else
    {
      ...
      else
      {
        ...
        {
          if(trace(checkSquare(sudoku, cellX, cellY, checkNum) 
  ...
  private static boolean checkRow(int[][] sudoku, int rowY, int toCheck)
  {
    ...
    for(int x = 0; x < 9; x++)
    {
      ...
      if (trace(toCheck == sudoku[rowY][x]))
      {
        return false;
      }
    }
  ...
  private static boolean checkCol(int[][] sudoku, int colX, int toCheck)
  {
    ...
    for(int y = 0; y < 9; y++)
    {
      ...
      if (trace(toCheck == sudoku[y][colX]))
      {
        return false;
      }
    }
