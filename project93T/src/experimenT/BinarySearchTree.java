...
private Node root;
    ...
    public void insert(String key) {
        if(functionF(root==null)) root= new Node(key);            
        else insert(root, key);
    }

    private void insert(Node cur, String key) {
        if (functionF(cur.value>=Integer.parseInt(key))){
            if(functionF(cur.left!=null)) insert(cur.left, key);
            else  cur.left= new Node(key); 
        }
        else{
            if(functionF(cur.right!=null)) insert(cur.right, key);
    ...
    public void printPrettyTree(){
        ...
        while(root!=null){
            ...
            for(; root!=null; root=root.next){
                if(functionF(nextLvl==null)) nextLvl=root.left!=null?root.left:root.right;
                int space=spaceAhead.get(root);
                row=rows.get(root);
                putNumberInLine(prettyTree, row, space, root.value);      
                if(functionF(root.left!=null)){
                    if(functionF(prev!=null)) prev.next=root.left;
                    prev=root.left;
                ...
                if(functionF(root.right!=null)){
                    if(functionF(prev!=null)) prev.next=root.right;
                    prev=root.right;
                    //generating the slashes
                    int r=row+1;
    ...
    public boolean subtree(BinarySearchTree s){
        if(functionF(s==null)) return true;
        return subtree(root, s.root);
    }
    
        private boolean subtree(Node t1, Node t2){
            if(functionF(t2!=null)) return true;//fault
            if(functionF(t1==null)) return false;
...
new BinarySearchTree().mainNonStatic();
    ...
    public void mainNonStatic() {
        ...
        try(PrintWriter out=new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Klant\\workspace11\\project93T\\results.txt",true)))){out.println((b0 == true));}catch(IOException ioException){} 
